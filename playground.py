...
def spacyTest():
    ...
    #doc = nlp(u'''he ran. she ran. the car belongs to him. the car is hers. all by herself. all by himself.''')
    ...
    # for ent in doc.ents:
    #     print('{}, {}, {} to {}'.format(ent.text, ent.label_, ent.start, ent.end))
    ...
    clauses = [doc[c.left_edge.i: c.right_edge.i + 1].text for word in doc if word.dep_ in ('ROOT') for c in word.children]
    printList(clauses)
    # for word in doc:
...
def printTree(q):
        ...
        dParse(q)
        print([doc[sent.left_edge.i: sent.right_edge.i + 1].text for sent in next(doc.sents).root.children][-2])
